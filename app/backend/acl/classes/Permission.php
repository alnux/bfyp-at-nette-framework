 ...
 * Edit 2015 Leonardo Allende <alnux@ya.ru>
 */
...
class Permission extends Object implements IAuthorizator
{
	...
	/**
	 * Returns all Parents Resource on a array.
	 * @return array
	 */
	public function getResourceParents($resource)
	{
		$this->checkResource($resource);
		$this->getResourceParent($resource, $parents);
		return explode(" ", $parents);
	}

	/**
	 * Get a parent of resource
	 * @param  String $resource 
	 * @param  String $parents
	 * @return String Return all parents
	 */
	private function getResourceParent($resource,&$parents)
	{
		if($this->resources[$resource]['parent'] !== NULL)
		{
			$parent = $this->resources[$resource]['parent'];
			$parents.= $this->resources[$resource]['parent'].' ';
			$this->getResourceParent($parent,$parents);
		}
